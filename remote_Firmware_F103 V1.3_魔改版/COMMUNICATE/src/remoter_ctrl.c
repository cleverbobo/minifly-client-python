...
void sendRmotorCmd(u8 cmd, u8 data)
{
	if(radioinkConnectStatus() == false)
		...
		return;
	atkp_t p;//创建通讯数据的实例
	p.msgID = DOWN_REMOTOR;//该通讯数据为下行的遥控指令（遥控器为上，四轴为下，所以是下行遥控指令）
	p.dataLen = 3;//数据长度为3
	p.data[0] = REMOTOR_CMD;//该指令为遥控命令
	p.data[1] = cmd;
	p.data[2] = data;
	radiolinkSendPacketBlocking(&p);//发送数据
}
/*发送遥控控制数据*/
void sendRmotorData(u8 *data, u8 len)
{
	if(radioinkConnectStatus() == false)
		return;
	atkp_t p;
	p.msgID = DOWN_REMOTOR;//下行遥控数据
	p.dataLen = len + 1;//数据头为数据类型
	p.data[0] = REMOTOR_DATA;//该数据为遥控数据
